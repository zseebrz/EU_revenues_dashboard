...
#color calculation
#(trans_count - min_trans_count) / (max_trans_count - min_trans_count + 0.00001)
...
def graphviz_visualization(activities_count, dfg, image_format="png", measure="frequency",
    ...
    viz = Digraph("", filename=filename.name, engine='neato', graph_attr={'bgcolor': 'transparent'})
    ...
    for act in activities_to_include:
        if "frequency" in measure and act in activities_count_int:
            ...
            activities_map[act] = act
        else:
            viz.node(act, act + " (" + human_readable_stat(soj_time[act]) + ")", fontsize=font_size)
            activities_map[act] = act
    ...
    for edge in dfg_edges:
        ...
        viz.edge((edge[0]), (edge[1]), label=label, penwidth=str(penwidth[edge]), fontsize=font_size)
